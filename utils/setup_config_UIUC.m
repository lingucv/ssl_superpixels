function p = setup_config_UIUC()
% configuration parameters for the UIUC shadow data 
...
p.dataset_name = 'UIUC'; % dataset name, used to find images lists
...
p.pos_sample_strategy = 'none';

p.pos_sample_distance = 0;

% p.neg_sample_distance = 0;
...
data_dir = 'E:\Projects\PA_ImPACT\Data\UIUC_Shadow\Our_train\';

gt_dir = 'E:\Projects\PA_ImPACT\Data\UIUC_Shadow\Our_train\';
...
for i_img = 1 : 32
    ...
    trn_img_list{i_img} = [data_dir  num2str(img_list(i_img)) '.png'];
    
    trn_gt_list{i_img} = [gt_dir 'annt_' num2str(img_list(i_img)) '.mat'];
...
data_dir = 'E:\Projects\PA_ImPACT\Data\UIUC_Shadow\Our_test\original\';

gt_dir = 'E:\Projects\PA_ImPACT\Data\UIUC_Shadow\Our_test\gt\';
...
dir_img_list = dir([data_dir '*.jpg']);

for i_img = 1 : length(dir_img_list)
   
    img_list{i_img} = dir_img_list(i_img).name;
...
for i_img = 1 : length(img_list)
    ...
    tst_img_list{i_img} = [data_dir  img_list{i_img}];
    
    tst_gt_list{i_img} = [gt_dir img_list{i_img}(1:end - 3) 'png'];
    
 %   I = imread(tst_img_list{i_img});
    
  %  gt_img = imread([tst_gt_list{i_img}(1:end - 3) 'png']);
...
%training samples
if(p.debug )
    p.neg_sample_no = 200; %number of samples far from centerlines
    p.pos_sample_no = 200; %number of samples close to centerlines
