function p = setup_config_Xray_stereo_large_image_only()
% configuration parameters for X ray vessel data 

% include the large image only
...
p.dataset_name = 'Xray_vessel'; % dataset name, used to find images lists
...
p.pos_sample_distance = 3;

p.neg_sample_distance = 5;
...
data_dir = '..\Data\Xray_vessel/data/';

gt_dir = '..\Data\Xray_vessel\binary input/';


% file_order = randperm(9);
% 
% trn_img_order = file_order(1:6);
% 
% tst_img_order = file_order(7:9);


% file_list_trn = dir([data_dir 'left/*.jpg']);


img_list = 3 : 23;

selected_imgs = [1,2,3,5,9,10,12,13,15,16,17,18,19,20,21];

img_list = img_list(selected_imgs);


for i_img = 1 : length(img_list)
    
    trn_img_list{i_img} = [data_dir 'left/' num2str(img_list(i_img)) '.jpg'];
    
    trn_gt_list{i_img} = [gt_dir 'left/' num2str(img_list(i_img)) '.png'];
...
for i_img = 1 : length(img_list)
    ...
    tst_img_list{i_img} = [data_dir 'right/'  num2str(img_list(i_img)) '.jpg'];
    
    tst_gt_list{i_img} = [gt_dir 'right/' num2str(img_list(i_img)) '.png'];
...
if(p.debug )
    ...
    p.neg_sample_no = 200000; %number of samples far from centerlines
    p.pos_sample_no = 200000; %number of samples close to centerlines
