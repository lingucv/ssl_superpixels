function forest = SSL_forestTrain_SLIC( data, hs, varargin )
...
%             
%             
%             knn_hs = KNN_label2unlabel_n(data1,hs1,udata1(img_idxs == i_img,:));
%             
%             
%             nll = nll1 + nll2;
%             
...
%             nur = nur1 + nur2;
%             
%             nul =  nul / (nul(1) + nur(1)) * (nll(1) + nlr(1));
% 
%             nur =  nur / (nul(1) + nur(1)) * (nll(1) + nlr(1));
%             
%             
%            abs(nll - nul) + abs(nlr - nur)
%            
%            subplot(2,1,1);
% 
%            plot(1:45,gain_l,'r',1:45,gain_u,'g',1:45,gain_single_img,'b');
% 
%            subplot(2,1,2);
%             
%            plot(1:45,abs(nll - nul) + abs(nlr - nur));
%            
%             
% %             knn_distr = mean(knn_hs == 1);
% %             
% %             knn_hs(knn_distr > 0.5) = 1;
% %             
% %             knn_hs(knn_distr < 0.5) = 2;
% %             
% %             mean(knn_hs(3,:)' ~= uhs1(img_idxs == i_img))
% 
%             mean(knn_hs ~= uhs1(img_idxs == i_img))
%             
%             [gain_knn,nkl1,nkl2,nkr1,nkr2] = evaluate_argument_data...
%                 (udata1(img_idxs == i_img,:),knn_hs,1 : nftrs,thr_l);
%             
