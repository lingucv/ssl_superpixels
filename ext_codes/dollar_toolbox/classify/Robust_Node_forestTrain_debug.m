function forest = Robust_Node_forestTrain_debug( data, hs, udata, uhs, varargin )
...
while( k < K )
    ...
    save('tmp_RN_data.mat','data1','hs1','datau1','uhs1');
...
% pm_range = [min(udata1(:)),max(udata1(:))];
% 
% X = pm_range(1) : (pm_range(2) - pm_range(1)) / 255 : pm_range(2);
% 
% X = double(X);
% 
% 
% udata_d = floor((udata1 - pm_range(1)) / ((pm_range(2) - pm_range(1)) / 255)) + 1;
...
[fid,thr,gain] = forestFindThr(data1,hs1,ones(size(hs1),'single'),order1,2,0);
...
if(~pure)
    ...
    for iter = 1 : n_iter
        ...
        [fid,thr,gain] = forestFindThr(udata1,uhs_est,ones(size(uhs_est),'single'),uorder1,2,0);
