function forest = Wilson_forestTrain_fix_interval( data, hs, udata, uhs, varargin )
...
% for the sake of implementation details, now assume the features are
% restricted to the [0 255] and the steps of the treshold selection is
% fixed
...
dfs={ 'M',1, 'H',[], 'N1',[], 'F1',[], 'split','gini', 'minCount',1, ...
  ...
  'record',0};
[M,H,N1,F1,splitStr,minCount,minChild,maxDepth,dWts,fWts,discretize,SSL,debug_mode,label_only,record] = ...
...
prmTree = {H,F1,minCount,minChild,maxDepth,fWts,split,discretize,SSL,debug_mode,label_only,record};
for i=1:M
...
[H,F1,minCount,minChild,maxDepth,fWts,split,discretize,SSL,debug_mode,label_only,...
    record]=deal(prmTree{:});
...
thr_set = [-1,  round(0 : 255 / 97 : 255) 256];
...
%     d = unique(d);
    ...
    pdfl1 = histc(data1(hs1 == 1,ifid),thr_set);
    ...
    pdfl2 = histc(data1(hs1 == 2,ifid),thr_set);
...
switch tree_mode
   ...
   if(debug_mode)
       ...
       pdfu1 = histc(udata1(uhs1 == 1,ifid),thr_set);
       ...
       pdfu2 = histc(udata1(uhs1 == 2,ifid),thr_set);
...
%        
       if(debug_mode && ((Nl1 + Nl2) < 200))
           ...
           plot(1:100,g_left + g_right - gi_w,'r',1:100,g_left_u + g_right_u - gi_u,'g',...
               1:100,g_left_l + g_right_l - gi_l,'b');
           
           subplot(3,1,2);
           
           plot(1:100,cdfl,'r',1:100,cdfun,'g');
           
           subplot(3,1,3);
           
           plot(1:100,pl1,'r',1:100,pul1,'g');
