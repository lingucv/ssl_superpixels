function forest = SSL_forestTrain_Wilson( data, hs, udata, uhs, varargin )
...
dfs={ 'M',1, 'H',[], 'N1',[], 'F1',[], 'split','gini', 'minCount',1, ...
  ...
  'record',0,'optimal',0,'expSSL',0,'Wilson',1};
[M,H,N1,F1,splitStr,minCount,minChild,maxDepth,dWts,fWts,discretize,SSL,...
    debug_mode,record,optimal,expSSL,Wilson] = ...
...
[H,F1,minCount,minChild,maxDepth,fWts,split,discretize,SSL,debug_mode,...
    record,optimal,expSSL,Wilson]=deal(prmTree{:});
...
while( k < K )
    ...
    if(tree_mode == 12)
        ...
        [hs_KNN,hs_KNN_dis] = KNN_label2unlabel(data(dids1,fid_knn),hs1,udata(didus1,fid_knn));
        
        hs_KNN_dis_sort = sort(hs_KNN_dis);
        
        thr_KNN = hs_KNN_dis_sort(ceil(0.01 * length(hs_KNN_dis_sort)));
        
        err_KNN1 = mean(hs_KNN ~= uhs1);
        
%         hs1_KNN_dis_sort = sort(hs_KNN_dis(hs_KNN == 1));
%         
%         hs2_KNN_dis_sort = sort(hs_KNN_dis(hs_KNN == 2));
        
%         thr_KNN1 = hs1_KNN_dis_sort(ceil(0.1 * length(hs1_KNN_dis_sort)));
%         
%         thr_KNN2 = hs2_KNN_dis_sort(ceil(0.1 * length(hs2_KNN_dis_sort)));
%         
%         
%         KNN_mask = ((hs_KNN_dis' < thr_KNN1) & (hs_KNN == 1)) | (((hs_KNN_dis' < thr_KNN2) & (hs_KNN == 2)));

        KNN_mask = (hs_KNN_dis' < thr_KNN);
        
% 
% %         
%         err_KNN2 = mean(hs_KNN(KNN_mask) ~= uhs1(KNN_mask));
%         
%         [err_KNN1,err_KNN2]
%        
%         Nl1 = sum(hs1 == 1);
...
%         Nl2 = sum(hs1 == 2);
%         
%         KNN_hs1_idx = find(KNN_mask & (hs_KNN == 1));
%         
%         KNN_hs2_idx = find(KNN_mask & (hs_KNN == 2));
%         
%         
%         Nk1 = length(KNN_hs1_idx);
%         
%         Nk2 = length(KNN_hs2_idx);
%         
%         
%         p12 = Nl1 / Nl2;
%         
...
%         if((Nk1 / Nl1) > (Nk2 / Nl2))
%         
%             Nk1 = min(ceil(Nk2 / p21),Nk1);
%             
%         else
%             
%             Nk2 = min(ceil(Nk1 / p12),Nk2);
%             
%         end
%         
%         KNN_hs1_idx = KNN_hs1_idx(1:Nk1);
%         
%         KNN_hs2_idx = KNN_hs2_idx(1:Nk2);
%         
%         
%         KNN_mask2 = zeros(size(uhs1));
%         
%         KNN_mask2([KNN_hs1_idx;KNN_hs2_idx]) = 1;
%         
%         KNN_mask2 = logical(KNN_mask2);
...
%         KNN_mask3_idx = randi(length(KNN_mask2),ceil(sum(KNN_mask2)),1);
%         
%         KNN_mask3 = ones(size(KNN_mask2));
%         
% %         KNN_mask3(KNN_mask3_idx) = 1;
% %         
%         KNN_mask3 = logical(KNN_mask3);
        
        
        
        [fid,thr,gain] = SSLforestFindThr_KNN(data1,hs1,datau1(KNN_mask,:)...
            ,uhs1(KNN_mask),hs_KNN(KNN_mask),tree_mode);
...
function [fid,thr,gain] = SSLforestFindThr(data1,hs1,udata1,uhs1,tree_mode,debug_mode)
...
%             hs_KNN = KNN_label2unlabel(data1,hs1,udata1);
            ...
            gain_Wilson = evaluate_argument_data_Wilson(data1,hs1,udata1,1 : nftrs,thr_l);
            
%             gain_u = evaluate_argument_data(udata1,uhs1,1 : nftrs,thr_l);
            
            [~,fid] = max(gain_Wilson);
%             
%             plot(1:45,gain_Wilson,'r',1:45,gain_u,'g',1:45,gain_l,'b');
...
function [fid,thr,gain] = SSLforestFindThr_KNN(data1,hs1,udata1,uhs1,hs1_KNN,tree_mode)
